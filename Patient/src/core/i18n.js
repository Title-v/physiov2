...
export const STRINGS = {
  en: {
    ...
    // Auth / welcome (V2)
    welcomeTitle: 'Welcome to PhysioAI', welcomeSub: 'AI physiotherapy that guides you at home',
    login: 'Log in', register: 'Create account', guest: 'Try without an account',
    fullName: 'Full name', email: 'Email', password: 'Password', confirmPassword: 'Confirm password',
    noAccount: "Don't have an account?", haveAccount: 'Already have an account?',
    signUpLink: 'Sign up', loginLink: 'Log in', logout: 'Log out', greeting: 'Hi, {name}',
    errRequired: 'Please fill in all fields', errInvalid: 'Wrong email or password',
    errExists: 'This email is already registered', errMatch: 'Passwords do not match',
    errApiConfig: 'The production API is not configured. Set EXPO_PUBLIC_API_BASE.',
    errRole: 'This account cannot use the patient app',
    errServer: 'Could not reach the server',
    landingTitle: 'AI physiotherapy at home',
    landingSub: 'For disabled persons in Thailand · Pose analysis runs on your device — no video ever leaves it.',
    ...
    myPlan: 'My plan', extras: 'Extra exercises', extrasHint: 'Optional · not part of treatment',
    healthRomCategory: 'ROM exercise presets',
    todayProgress: "Today's progress", planAllDone: 'All done for today 🎉', planDoneOf: '{done}/{total} done today',
    ...
    exd_rom_left_shoulder: 'Measure controlled range of motion for your left shoulder',
    exd_rom_right_shoulder: 'Measure controlled range of motion for your right shoulder',
    exd_rom_left_elbow: 'Measure controlled range of motion for your left elbow',
    exd_rom_right_elbow: 'Measure controlled range of motion for your right elbow',
    exd_rom_left_hip: 'Measure controlled range of motion for your left hip',
    exd_rom_right_hip: 'Measure controlled range of motion for your right hip',
    exd_rom_left_knee: 'Measure controlled range of motion for your left knee',
    exd_rom_right_knee: 'Measure controlled range of motion for your right knee',
    exd_rom_back: 'Measure controlled range of motion for your back',
    exd_rom_neck: 'Measure controlled range of motion for your neck',
  ...
  th: {
    ...
    // Auth / welcome (V2)
    welcomeTitle: 'ยินดีต้อนรับสู่ PhysioAI', welcomeSub: 'AI กายภาพบำบัดที่นำคุณฝึกที่บ้าน',
    login: 'เข้าสู่ระบบ', register: 'สมัครสมาชิก', guest: 'ทดลองใช้แบบไม่ล็อกอิน',
    fullName: 'ชื่อ-นามสกุล', email: 'อีเมล', password: 'รหัสผ่าน', confirmPassword: 'ยืนยันรหัสผ่าน',
    noAccount: 'ยังไม่มีบัญชี?', haveAccount: 'มีบัญชีอยู่แล้ว?',
    signUpLink: 'สมัคร', loginLink: 'เข้าสู่ระบบ', logout: 'ออกจากระบบ', greeting: 'สวัสดี, {name}',
    errRequired: 'กรุณากรอกข้อมูลให้ครบ', errInvalid: 'อีเมลหรือรหัสผ่านไม่ถูกต้อง',
    errExists: 'อีเมลนี้ถูกใช้สมัครแล้ว', errMatch: 'รหัสผ่านไม่ตรงกัน',
    errApiConfig: 'ยังไม่ได้ตั้งค่า production API กรุณาตั้ง EXPO_PUBLIC_API_BASE',
    errRole: 'บัญชีนี้ใช้กับแอปผู้ป่วยไม่ได้',
    errServer: 'เชื่อมต่อเซิร์ฟเวอร์ไม่สำเร็จ',
    landingTitle: 'กายภาพบำบัดที่บ้านด้วย AI',
    landingSub: 'สำหรับผู้พิการในประเทศไทย · วิเคราะห์ท่าทางบนเครื่องของคุณ — วิดีโอไม่ถูกส่งออกไปไหน',
    ...
    myPlan: 'แผนของฉัน', extras: 'ท่าเสริม', extrasHint: 'ทำเสริมได้ · ไม่นับในการรักษา',
    healthRomCategory: 'ท่า ROM สำหรับฝึก',
    todayProgress: 'ความคืบหน้าวันนี้', planAllDone: 'ครบแล้ววันนี้ 🎉', planDoneOf: 'ทำแล้ว {done}/{total} วันนี้',
...
// ── Language state (React Native: in-memory + AsyncStorage, no DOM) ──
const LANG_KEY = 'physioai.v2.lang';
let _lang = 'th';                      // Thai-first (target users); toggle to 'en'
const _listeners = new Set();

/** Load the saved language once at startup (call from App before first render). */
export async function loadLang() {
  try { const v = await AsyncStorage.getItem(LANG_KEY); if (v) _lang = v; } catch {}
  return _lang;
}
...
export function setLang(lang) {
  ...
  _listeners.forEach((cb) => { try { cb(lang); } catch {} });
}
